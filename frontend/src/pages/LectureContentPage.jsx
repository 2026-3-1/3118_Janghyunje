    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center px-4">
      <div className="bg-white dark:bg-[#13161e] rounded-2xl p-6 w-full max-w-sm space-y-4 shadow-2xl">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-gray-400 hover:text-gray-700 dark:hover:text-white text-lg">✕</button>
        </div>
        ...
        <div className="flex items-center justify-center py-2">
          <div className="relative w-16 h-16">
            <svg className="w-16 h-16 -rotate-90" viewBox="0 0 64 64">
              <circle cx="32" cy="32" r="28" fill="none" stroke="currentColor" className="text-gray-100 dark:text-[#2a2d3e]" strokeWidth="4" />
              <circle cx="32" cy="32" r="28" fill="none" stroke="currentColor" className="text-brand-500 transition-all duration-1000"
                strokeWidth="4" strokeDasharray={`${2 * Math.PI * 28}`}
                strokeDashoffset={`${2 * Math.PI * 28 * (1 - countdown / 10)}`} strokeLinecap="round" />
            </svg>
            <span className="absolute inset-0 flex items-center justify-center text-lg font-extrabold text-brand-500">{countdown}</span>
          </div>
        </div>
        <p className="text-xs text-center text-gray-400 dark:text-[#6b7280]">{countdown}초 후 자동으로 다음 강의로 이동합니다</p>
        <div className="flex gap-2">
          <button onClick={onClose} className="flex-1 py-2.5 bg-gray-100 dark:bg-[#1a1d2e] text-gray-600 dark:text-slate-300 font-semibold text-sm rounded-xl hover:bg-gray-200 transition-colors">머무르기</button>
          <button onClick={onNext} className="flex-1 py-2.5 bg-brand-500 hover:bg-brand-600 text-white font-semibold text-sm rounded-xl transition-colors">다음 강의 →</button>
        </div>
      </div>
    </div>
  )
...
export default function LectureContentPage() {
  ...
  const handleSelectContent = (c) => { setNextModal(null); setSelected(c); setNewComment('') }
  const handleNextContent    = () => { setNextModal(null); if (nextModal) handleSelectContent(nextModal) }
  ...
  const handleComment = async () => {
    ...
    try {
      ...
      setNewComment('')
    } catch (err) { alert(err.response?.data?.message || '댓글 작성에 실패했습니다.') }
    finally { setCommenting(false) }
  }

  const handleDeleteComment = async (commentId) => {
    if (!window.confirm('댓글을 삭제하시겠습니까?')) return
    try { await api.delete(`/comments/${commentId}`); setComments(prev => prev.filter(c => c.id !== commentId)) } catch {}
  }
  ...
  return (
    <div className="flex h-[calc(100vh-52px-60px)] overflow-hidden">
      {nextModal && <NextContentModal nextContent={nextModal} onNext={handleNextContent} onClose={() => setNextModal(null)} />}
          ...
          {lectureProgress && (
            <div className="mt-2 space-y-1">
              <div className="flex justify-between text-xs text-gray-400 dark:text-[#6b7280]">
                <span>전체 진도</span>
                <span className="font-medium text-gray-700 dark:text-slate-300">{lectureProgress.percent}%</span>
              </div>
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      ...
                      {!isDone && watchedPct > 0 && (
                        <div className="flex-1 h-1 bg-gray-100 dark:bg-[#2a2d3e] rounded-full overflow-hidden">
                          <div className="h-full bg-brand-400 rounded-full" style={{ width: `${watchedPct}%` }} />
                        </div>
                      )}
                    </div>
            ...
            {selected.type === 'material' && (
              <div className="bg-white dark:bg-[#13161e] border border-gray-100 dark:border-[#1e2235] rounded-xl p-6 flex items-center gap-4">
                ...
                <a href={selected.url} target="_blank" rel="noopener noreferrer"
                  className="px-4 py-2 bg-brand-500 hover:bg-brand-600 text-white text-sm font-semibold rounded-lg transition-colors shrink-0">열기</a>
              </div>
            )}
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 space-y-2">
                    <textarea value={newComment} onChange={e => setNewComment(e.target.value)}
                      ...
                      className="w-full bg-gray-50 dark:bg-[#0d0f14] border border-gray-200 dark:border-[#2a2d3e] rounded-lg px-3 py-2.5 text-sm text-gray-800 dark:text-slate-200 placeholder:text-gray-300 dark:placeholder:text-[#4a5568] outline-none focus:border-brand-400 resize-none transition-colors" />
                  ...
                  <div key={c.id} className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {user?.id === c.user_id && (
                          <button onClick={() => handleDeleteComment(c.id)} className="ml-auto text-xs text-gray-300 hover:text-red-500 transition-colors">삭제</button>
                        )}
                      </div>
