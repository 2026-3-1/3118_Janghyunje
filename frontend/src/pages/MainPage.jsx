  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 space-y-10">
      ...
      <section className="bg-gradient-to-r from-brand-500 to-brand-700 rounded-2xl p-7 text-white">
        ...
        <p className="text-white/70 text-sm mb-5">검증된 코치와 함께 게임 실력을 빠르게 올리세요</p>
        ...
        <div className="flex items-center gap-2 bg-white/15 border border-white/30 rounded-xl px-4 py-2.5 max-w-md mb-5 focus-within:bg-white/20 transition-colors">
          ...
          <input type="text" placeholder="강의명, 코치명으로 검색..."
            value={keyword} onChange={e => setKeyword(e.target.value)}
            className="bg-transparent text-white placeholder:text-white/50 text-sm outline-none flex-1" />
          {keyword && (
            <button onClick={() => setKeyword('')} className="text-white/60 hover:text-white text-xs transition-colors">✕</button>
          )}
        </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => navigate('/lectures')} className="text-sm text-brand-500 hover:text-brand-600 transition-colors">
            전체 보기 →
          </button>
        </div>

        <div className="mb-4"><SearchFilter /></div>
