...
export default function LectureDetailPage() {
  ...
  const loadReviews = async () => {
    ...
    setReviews(r)
    if (user) {
      if (r.find(rv => rv.student_id === user.id)) setAlreadyReviewed(true)
    }
  }
  ...
  useEffect(() => {
    Promise.all([getLectureById(id), getReviewsByLectureId(id)])
      .then(([l, r]) => {
        setLecture(l)
        setReviews(r)
        if (user && r.find(rv => rv.student_id === user.id)) setAlreadyReviewed(true)
        const prev = JSON.parse(localStorage.getItem('recentLectures') || '[]')
        localStorage.setItem('recentLectures', JSON.stringify([l.id, ...prev.filter(rid => rid !== l.id)].slice(0, 5)))
      })
  ...
  useEffect(() => {
    ...
    api.get('/cart')
      .then(res => {
        setInCart((res.data.data || []).some(i => (i.lecture_id ?? i.id) === Number(id)))
      }).catch(() => {})
  }, [id, user])
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      await api.post('/reviews', { lecture_id: Number(id), rating: reviewRating, comment: reviewComment.trim() })
      ...
      if (err.response?.status === 409) { showToast('이미 후기를 작성하셨습니다.'); setAlreadyReviewed(true) }
      else showToast(err.response?.data?.message || '후기 등록에 실패했습니다.')
    } finally { setSubmittingReview(false) }
  }

  if (loading) return <LoadingScreen />

  const discountRate   = lecture.originalPrice ? Math.round((1 - lecture.price / lecture.originalPrice) * 100) : null
    ...
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-5">
      ...
      {toast.msg && (
        <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2.5
          ...
          {toast.msg}
          {!user && (
            <button onClick={() => navigate('/login')} className="ml-2 underline text-xs font-semibold">로그인하기</button>
          )}
        </div>
      )}
      ...
      <div className="bg-white dark:bg-[#13161e] border border-gray-100 dark:border-[#1e2235] rounded-xl p-5 space-y-4">
        {/* 가격 */}
        <div>
          <p className="text-xs text-gray-400 dark:text-[#6b7280] mb-1">수강료</p>
        ...
        <div className="flex flex-col gap-2.5">
            ...
            <>
              ...
              {inCart && (
                <button onClick={() => navigate('/cart')}
                  className="w-full py-1.5 text-xs text-brand-500 hover:text-brand-600 font-medium transition-colors">
                  장바구니 바로가기 →
                </button>
              )}
            </>
          ...
          {/* 신청 완료 상태 표시 */}
          {applicationStatus && !isMyLecture && applicationStatus !== 'approved' && (
            <button disabled className={`w-full py-3 rounded-xl font-bold text-sm border ${APPLICATION_STATUS[applicationStatus]?.cls}`}>
        ...
        <div className="bg-white dark:bg-[#13161e] border border-brand-200 dark:border-brand-500/30 rounded-xl p-5 space-y-4">
          ...
          <div>
            <textarea value={reviewComment} onChange={e => setReviewComment(e.target.value)}
              placeholder="수강 후기를 자유롭게 작성해주세요." rows={4} maxLength={500}
              className="w-full rounded-xl border border-gray-200 dark:border-[#2a2d3e] bg-gray-50 dark:bg-[#0d0f14]
                         ...
                         focus:border-brand-400 transition-colors placeholder:text-gray-300 dark:placeholder:text-[#4a5568]" />
          ...
          <button onClick={handleReviewSubmit} disabled={submittingReview}
            className="w-full py-2.5 rounded-xl font-bold text-sm bg-brand-500 hover:bg-brand-600 disabled:bg-brand-300 text-white transition-colors">
      ...
      {user && !isMyLecture && applicationStatus === 'approved' && alreadyReviewed && (
        <div className="bg-gray-50 dark:bg-[#13161e] border border-gray-100 dark:border-[#1e2235] rounded-xl p-4 text-center text-sm text-gray-400">
          ✓ 이미 후기를 작성하셨습니다.
        </div>
      )}
